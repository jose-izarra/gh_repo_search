            ...
            <div className="flex flex-col items-start gap-x-2 gap-y-3">
                <span className="flex items-center gap-x-2">
                    <h3
                        ...
                        >{(repo as RepoResult).name}</h3>
                    <span className="text-xs font-medium text-gray-500 border border-gray-300 rounded-full px-2 py-1">{(repo as RepoResult).visibility}</span>
                </span>
                <p className="text-base font-medium text-gray-600">{(repo as RepoResult).description}</p>
                <div className="flex items-center gap-x-4 mt-2">
                    <span className="flex items-center gap-x-1">
                        <Star className="size-4 text-yellow-500" />
                        <p className="text-sm font-medium text-gray-500">{(repo as RepoResult).stargazers_count}</p>
                    </span>
                    <span className="flex items-center gap-x-1">
                        <ForkIcon className="size-4 text-gray-500" />
                        <p className="text-sm font-medium text-gray-500">{(repo as RepoResult).forks_count}</p>
                    </span>
                    <span className="flex items-center gap-x-1">
                        <Eye className="size-4 text-gray-500" />
                        <p className="text-sm font-medium text-gray-500">{(repo as RepoResult).watchers_count}</p>
                    </span>
                </div>
            </div>
            <div className="flex  flex-row md:flex-col md:justify-center  gap-x-3 py-1 min-w-52">
                <span className="flex gap-x-1 items-center">
                    <CodeXml className="size-4 text-gray-500" />
                    <p className="text-sm font-medium text-gray-600 ">{(repo as RepoResult).language || "Unknown"}</p>
                </span>
                <p className="text-sm font-medium text-gray-400 ">Last updated: {(repo as RepoResult).updated_at}</p>
            </div>
        </div>
    )
}
